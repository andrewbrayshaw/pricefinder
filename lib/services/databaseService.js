...
export async function createTableIfNotExists(db, keyMapping, retries = 3, delay = 2000) {
  try {
    ...
    if (/(latitude|longitude|area)/i.test(safeColumnName)) dataType = 'REAL';
    if (/(price|estimate|bedrooms|bathrooms|carspaces|garages|yearbuilt|postcode|addressnumber|streetid|suburbid|postcodeid|dayslisted|listingprice|salehistoryprice)/i.test(safeColumnName)) dataType = 'INTEGER';
...
export async function insertProperty(propertyData, keyMapping) {
  ...
  try {
    ...
    const values = Object.values(keyMapping).map(k => {
      ...
      return (val !== undefined && val !== null) ? val : null; //'N/A';
    });
