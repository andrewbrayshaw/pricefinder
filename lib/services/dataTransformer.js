import { keyMapping, excludeKeys } from '../config/keyMappings.js';
...
function flattenListingHistory(listingHistory) {
  ...
  if (listingHistory.length === 0) {
    console.log('Listing history is empty, assigning null to all listing history keys.');
    ...
    Object.keys(keyMapping).forEach((key) => {
      if (key.startsWith('data.property.detail.listingHistory')) {
        flattenedData[keyMapping[key] || key] = null; //'N/A'; // Use keyMapping, or fallback to the original key
      }
    });
  ...
  listingHistory.forEach((listingItem, index) => {
    Object.keys(listingItem).forEach((key) => {
      ...
      //flattenedData[mappedKey] = listingItem[key] !== undefined && listingItem[key] !== null ? listingItem[key] : null; //'N/A';
      flattenedData[mappedKey] = listingItem[key] !== undefined ? listingItem[key] : null; //'N/A';
    });
  });
...
function flattenSaleHistory(saleHistory) {
  ...
  if (saleHistory.length === 0) {
    console.log('Sales history is empty, assigning null to all listing history keys.');
  ...
  saleHistory.forEach((historyItem, index) => {
    Object.keys(historyItem).forEach((key) => {
      ...
      //flattenedData[mappedKey] = historyItem[key] !== undefined && historyItem[key] !== null ? historyItem[key] : null; //'N/A';
      flattenedData[mappedKey] = historyItem[key] !== undefined ? historyItem[key] : null; //'N/A';
    });
  });
...
export function buildPropertyData(json) {
  ...
  function recursiveExtract(data, path = '') {
    if (data && typeof data === 'object') {
      ...
      if (keyMapping[path]) { // use path instead of newpath
        //result[keyMapping[path]] = data !== undefined && data !== null ? data : null; // "N/A";
        result[keyMapping[path]] = data !== undefined ? data : null; // "N/A";
      }
  ...
  if (json && json.data && json.data.property && Array.isArray(json.data.property.detail.features)) {
    result["Features"] = json.data.property.detail.features.length > 0 
      ...
      : null //"N/A";  // If the array is empty, set as "N/A"
  }
